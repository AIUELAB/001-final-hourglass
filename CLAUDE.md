...
#### 品質・分析系（Phase 18-D追加）
- `/pdca` - PDCAガーディアン（品質分析・改善提案）
- `/codex-analyze` - AI協調分析（多視点統合分析）
- `/kairos` - KAIROS（機会検出・タイミング提案）
- `/rca` - RCA-Kaizen（根本原因分析・継続改善）

#### MCP管理系（Phase 18-C追加）
- `/mcp-profile` - MCPプロファイル切替
...
### MCPプロファイル（Phase 18-C）

コンテキスト消費に応じて選択可能：

| プロファイル | 消費 | 用途 |
|-------------|------|------|
| `minimal` | 3.0k | 通常開発（serena, codex, context7, ide） |
| `web` | 7.9k | Web検索（+ fetch, brave-search） |
| `scraping` | 30.0k | スクレイピング（+ firecrawl, playwright） |
| `full` | 30.8k | 全機能有効 |
...
Dev Container対応済み。httpx=0.27.0統一、Ollama分離で解決。

---

## 🔧 Git LFS設定（2025-11-20完了）

Git LFSで大容量ファイル（SESSION_LOG.md等）を管理。PR #4でマージ済み。
- 追跡設定: `.gitattributes`
- 50MB以上のファイルはLFS追跡を追加推奨
