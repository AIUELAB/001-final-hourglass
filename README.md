...
| URL | 説明 |
|-----|------|
| http://localhost:8000/ | ダッシュボード v3（推奨） |
| http://localhost:8000/v2 | ダッシュボード v2 |
...
問題が発生した場合は、詳細なガイドを参照してください：

- **[エピソードメインデータベース v3 起動ガイド](docs/EPISODE_DB_STARTUP_GUIDE.md)** - 完全なセットアップとトラブルシューティング
- **[データ分析レポート](docs/PHASE6_DATA_ANALYSIS_REPORT.md)** - 現在のデータベース状態（2,613件のエピソード）
- **[データ拡充ロードマップ](docs/DATA_EXPANSION_ROADMAP.md)** - 今後の拡充計画

### 主な機能

- **2,613件の実在人物エピソード** - 多様な分野・年代をカバー
- **366日完全対応** - 全日程にエピソード配置済み
- **高度な検索・フィルタ** - 分野、年代、キーワードで検索
- **統計ダッシュボード** - データの可視化と分析
- **自動更新** - 30秒ごとにデータベース更新をチェック
