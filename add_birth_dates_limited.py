#!/usr/bin/env python3
"""
WikipediaAPIから生年月日情報を抽出（制限付き高速版）
"""
...
def extract_birth_info_from_wikitext(wikitext):
    ...
    # YYYY年MM月DD日形式
    match = re.search(r'生年月日[^=]*=.*?(\d{4})年(\d{1,2})月(\d{1,2})日', wikitext[:2000])  # 最初の2000文字のみ
    if match:
        year = int(match.group(1))
        month = int(match.group(2))
        day = int(match.group(3))
...
def main():
    print("=" * 80)
    print("📅 Wikipedia APIから生年月日情報を抽出（制限付き高速版）")
    print("=" * 80)
    ...
    if len(target_df) > limit:
        print(f"⚠️ 制限モード: 最初の{limit}件のみ処理します")
