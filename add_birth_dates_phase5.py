#!/usr/bin/env python3
"""
WikipediaAPIから生年月日情報を抽出（フェーズ5 - 継続処理版）
"""
...
def main():
    print("=" * 80)
    print("📅 Wikipedia APIから生年月日情報を抽出（フェーズ5 - 継続処理）")
    print("=" * 80)

    # 前回の処理結果を入力として使用（フェーズ4の結果）
    input_file = 'ultra_think_WITH_BIRTH_DATES_PHASE4_20250916_220615.csv'
    ...
    # 次の100件を処理
    skip = 0  # フェーズ5も既に処理済みのものは除外されているので0からスタート
    limit = 100
    if len(target_df) > skip:
        print(f"⚠️ フェーズ5: {skip+1}〜{min(skip+limit, len(target_df))}件目を処理")
    ...
    output_file = f'ultra_think_WITH_BIRTH_DATES_PHASE5_{timestamp}.csv'
