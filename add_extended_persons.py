#!/usr/bin/env python3
"""
拡張人物リスト（教訓的価値のある人物含む）をデータベースに追加
"""
...
from extended_persons_with_lessons import ExtendedPersonsWithLessons
from special_category_evaluator_v2 import SpecialCategoryEvaluatorV2

class ExtendedPersonAdder:
    """拡張人物追加クラス"""
    ...
    def __init__(self, existing_database_file: str):
        ...
        self.extended_persons = ExtendedPersonsWithLessons()
    ...
    def add_all_persons(self, limit: int = None):
        ...
        if new_persons:
            ...
            # 出力ファイル
            output_file = f"database_extended_{self.timestamp}.csv"
            ...
            # 新規追加分のみも出力
            added_file = f"extended_added_persons_{self.timestamp}.csv"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='拡張人物追加処理')
    parser.add_argument('--input', type=str,
                       default='database_expanded_20250910_045159.csv',
                       help='既存データベースファイル')
    ...
    # 処理実行
    adder = ExtendedPersonAdder(args.input)
