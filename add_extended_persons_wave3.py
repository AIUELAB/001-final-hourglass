#!/usr/bin/env python3
"""
拡張人物リスト第3波をデータベースに追加（ChatGPT提案基づく）
目標: 残り421名に向けて62名を追加
"""
...
from extended_persons_wave3_from_chatgpt import ExtendedPersonsWave3FromChatGPT
from special_category_evaluator_v2 import SpecialCategoryEvaluatorV2

class ExtendedPersonAdderWave3:
    """拡張人物追加クラス第3波（ChatGPT提案）"""
    ...
    def __init__(self, existing_database_file: str):
        ...
        self.extended_persons = ExtendedPersonsWave3FromChatGPT()
    ...
    def check_existing(self, name: str) -> bool:
        ...
        # スペースを除外して比較（「村木 厚子」と「村木厚子」を同一視）
        base_name_no_space = base_name.replace(' ', '').replace('　', '')
        for existing_name in self.existing_df['name'].values:
            existing_clean = str(existing_name).split('(')[0].strip().replace(' ', '').replace('　', '')
            if base_name_no_space == existing_clean:
                return True
        return False
    ...
    def process_person(self, name: str, occupation: str, 
        ...
        # ChatGPT提案の特別加点（ストーリー性・教訓的価値）
        if category in ['防災', '医療', '技術', '教育', '国際協力']:
            final_score = min(10.0, final_score + 0.5)  # 社会的重要性加点
            reason += " (ChatGPT推奨カテゴリ)"
        ...
        person_data = {
            ...
            'source': 'ChatGPT提案Wave3'  # データソースを明記
        }
    ...
    def add_all_persons(self, limit: int = None):
        ...
        logger.info("=" * 60)
        logger.info("🎯 拡張人物追加処理開始【第3波 - ChatGPT提案】")
        logger.info("=" * 60)
        ...
        # カテゴリサマリー表示
        category_summary = self.extended_persons.get_category_summary()
        logger.info("\n📋 カテゴリ別内訳:")
        ...
        if new_persons:
            ...
            # 出力ファイル
            output_file = f"database_extended_wave3_{self.timestamp}.csv"
            ...
            # 新規追加分のみも出力
            added_file = f"extended_added_persons_wave3_{self.timestamp}.csv"
    ...
    def show_statistics(self):
        ...
        logger.info("\n" + "=" * 60)
        logger.info("📊 追加処理統計【第3波 - ChatGPT提案】")
        logger.info("=" * 60)
        ...
        logger.info(f"第2波: 21名追加済み")
        logger.info(f"第3波: {self.stats['added_successfully']}名追加")
        logger.info(f"合計: {58 + 21 + self.stats['added_successfully']}名追加")
        logger.info(f"目標500名に対する達成率: {((58 + 21 + self.stats['added_successfully']) / 500) * 100:.1f}%")
        
        # ChatGPT提案の価値
        logger.info("\n💡 ChatGPT提案の成果:")
        logger.info("  ✅ 「挫折と再起」のストーリー性を持つ人物を追加")
        logger.info("  ✅ 生活密着型（防災・医療・介護）を強化")
        logger.info("  ✅ 現場職のプロフェッショナルを新規追加")
        logger.info("  ✅ 女性パイオニア・歴史的改革者を網羅")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='拡張人物追加処理【第3波 - ChatGPT提案】')
    parser.add_argument('--input', type=str,
                       default='database_extended_wave2_20250910_095433.csv',
                       help='既存データベースファイル')
    ...
    # 処理実行
    adder = ExtendedPersonAdderWave3(args.input)
