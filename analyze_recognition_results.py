...
def analyze_recognition_results():
    ...
    # 削除候補を特定
    delete_candidates = df[df['final_score'] <= DELETE_THRESHOLD].copy()
    keep_records = df[df['final_score'] > DELETE_THRESHOLD].copy()
    ...
    if len(delete_candidates) > 0:
        print("\n📋 削除候補の詳細:")
        print(f"  スコア分布:")
        for score in [0.5, 1.0, 1.5, 2.0, 2.5, 3.0]:
            count = len(delete_candidates[delete_candidates['final_score'] == score])
            if count > 0:
    ...
    # 有名作品のキャラクター確認
    famous_characters = ['竈門炭治郎', '孫悟空', 'ドラえもん', 'ピカチュウ', 'ルフィ']
    print("\n  有名キャラクターの確認:")
    for char_name in famous_characters:
        ...
        if not char_record.empty:
            ...
            status = "✅ 保持" if score > DELETE_THRESHOLD else "❌ 削除候補"
            print(f"    {char_name}: スコア {score} - {status}")
    ...
    summary = {
        ...
        "fictional_characters": {
            "total": len(fictional),
            "delete": len(fictional_delete),
            "keep": len(fictional_keep)
        },
    ...
    return delete_candidates, keep_records

if __name__ == "__main__":
    delete_candidates, keep_records = analyze_recognition_results()
