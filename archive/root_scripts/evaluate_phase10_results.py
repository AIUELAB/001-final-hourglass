#!/usr/bin/env python3
"""
Phase 10.6: Phase 10結果評価

Phase 10改善後のエピソードを評価し、Phase 9との比較を行う。
"""
...
def compare_with_phase9(phase10_stats: Dict, phase9_stats_path: str) -> Dict:
    """Phase 9との比較"""
    with open(phase9_stats_path, 'r', encoding='utf-8') as f:
        phase9_stats = json.load(f)

    comparison = {
        "pass_rate": {
            "phase9": phase9_stats["pass_rate"],
            "phase10": phase10_stats["pass_rate"],
            "change": phase10_stats["pass_rate"] - phase9_stats["pass_rate"]
        },
        "average_score": {
            "phase9": phase9_stats["average_score"],
            "phase10": phase10_stats["average_score"],
            "change": phase10_stats["average_score"] - phase9_stats["average_score"]
        },
        "average_social_impact": {
            "phase9": phase9_stats["average_social_impact"],
            "phase10": phase10_stats["average_social_impact"],
            "change": phase10_stats["average_social_impact"] - phase9_stats["average_social_impact"]
        },
        "passed_count": {
            "phase9": phase9_stats["passed"],
            "phase10": phase10_stats["passed"],
            "change": phase10_stats["passed"] - phase9_stats["passed"]
        }
    }
...
def main():
    """メイン実行"""
    print("=" * 80)
    print("Phase 10.6: Phase 10結果評価")
    print("=" * 80)

    # Phase 10改善後のエピソードを読み込み
    print("\n📂 Phase 10エピソードを読み込み中...")
    episodes = load_episodes("episodes_phase10_complete.csv")
    ...
    # Phase 9との比較
    print("📊 Phase 9との比較中...")
    comparison = compare_with_phase9(stats, "episodes_phase9_evaluation_stats.json")
    ...
    # 評価結果CSV
    with open("episodes_phase10_evaluation.csv", 'w', encoding='utf-8-sig', newline='') as f:
        fieldnames = ['episode_id', 'person_name', 'episode_age',
                     'total_score', 'social_impact_score', 'passed', 'character_count']
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(results)
    print(f"✅ 評価CSV保存: episodes_phase10_evaluation.csv")

    # 統計JSON
    with open("episodes_phase10_evaluation_stats.json", 'w', encoding='utf-8') as f:
        json.dump(stats, f, ensure_ascii=False, indent=2)
    print(f"✅ 統計JSON保存: episodes_phase10_evaluation_stats.json")

    # 比較JSON
    with open("episodes_phase10_comparison.json", 'w', encoding='utf-8') as f:
        json.dump(comparison, f, ensure_ascii=False, indent=2)
    print(f"✅ 比較JSON保存: episodes_phase10_comparison.json")

    print(f"\n" + "=" * 80)
    print(f"✅ Phase 10.6 評価完了")
    print(f"=" * 80)
