...
from app.utils.score_calculator import calculate_text_similarity, is_similar_episode, normalize_person_name
...
class DataQualityManager:
    ...
    def detect_duplicates(
        self, use_normalization: bool = True, check_text_similarity: bool = True, similarity_threshold: float = 0.8
    ) -> List[Dict]:
        """
        重複エピソードの検出（高度版）
        ...
        for key, episodes in seen.items():
            if len(episodes) > 1:
                ...
                if check_text_similarity:
                    # 類似グループを検出
                    similar_groups = self._find_similar_text_groups(episodes, similarity_threshold)
    ...
    def _find_similar_text_groups(self, episodes: List[Dict], threshold: float) -> List[List[Dict]]:
        """
    ...
    def detect_duplicates_simple(self) -> List[Dict]:
        ...
        Returns:
            重複情報のリスト
        """
        return self.detect_duplicates(use_normalization=False, check_text_similarity=False)
    ...
    def check_completeness(self) -> Dict:
        ...
        return {"total_episodes": total, "issues": issues, "issues_count": len(issues)}
