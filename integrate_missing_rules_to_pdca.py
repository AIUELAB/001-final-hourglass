#!/usr/bin/env python3
"""
PDCAガーディアンへ未実装ルールを統合するスクリプト
RULE_077-080, 157-159, 161-163を自動的に追加
"""

import re
from pathlib import Path

def add_violation_types(content):
    """ViolationType列挙型に新しい違反タイプを追加"""
...
"""
    ...
    # ViolationTypeクラスを探す
    pattern = r'(class ViolationType.*?:.*?\n)((?:    .*\n)+)'
    match = re.search(pattern, content, re.DOTALL)

    if match:
        class_def = match.group(1)
        existing_types = match.group(2)

        # 既に追加済みかチェック
        if "CONSECUTIVE_ID_MISJUDGMENT" not in existing_types:
            # 最後の定義の後に追加
            new_content = class_def + existing_types.rstrip() + "\n" + new_types + "\n"
            content = content[:match.start()] + new_content + content[match.end():]
            print("✅ ViolationType列挙型に10個の新しい違反タイプを追加")
        else:
            print("ℹ️ ViolationType列挙型は既に追加済み")

    return content


def add_check_methods(content):
    """チェックメソッドを追加"""
...
'''
    ...
    # check_episode_qualityメソッドを探してメソッドを追加
    # メソッドの最後の部分を探す
    pattern = r'(def check_episode_quality.*?\n(?:.*?\n)*?)(        return violations\n)'
    match = re.search(pattern, content, re.DOTALL)

    if match and 'check_consecutive_id_protection' not in content:
        # メソッドの最後のreturn文の前に追加
        method_body = match.group(1)
        return_stmt = match.group(2)

        # 新しいメソッドをクラス内に追加
        # PDCAGuardianクラスの最後を探す
        class_pattern = r'(class PDCAGuardian.*?:\n(?:.*?\n)*?)(^class |^def main|^if __name__|$)'
        class_match = re.search(class_pattern, content, re.MULTILINE | re.DOTALL)

        if class_match:
            class_body = class_match.group(1)
            after_class = class_match.group(2) if class_match.group(2) else ''

            new_content = class_body + new_methods + "\n" + after_class
            content = content[:class_match.start()] + new_content + content[class_match.end():]
            print("✅ 10個の新しいチェックメソッドを追加")
        else:
            print("❌ PDCAGuardianクラスが見つかりません")
    elif 'check_consecutive_id_protection' in content:
        print("ℹ️ チェックメソッドは既に追加済み")

    return content


def add_integration_calls(content):
    """check_episode_qualityメソッドに新しいチェックの呼び出しを追加"""
...
"""
    ...
    # check_episode_qualityメソッド内のreturn文を探す
    pattern = r'(def check_episode_quality.*?\n(?:.*?\n)*?)(        return violations\n)'
    match = re.search(pattern, content, re.DOTALL)

    if match and 'RULE_077' not in match.group(1):
        method_body = match.group(1)
        return_stmt = match.group(2)

        # return文の前に統合コードを追加
        new_method = method_body + integration_code + "\n" + return_stmt
        content = content[:match.start()] + new_method + content[match.end():]
        print("✅ check_episode_qualityメソッドに10個の新しいチェックを統合")
    elif 'RULE_077' in content:
        print("ℹ️ check_episode_qualityメソッドは既に統合済み")
...
def main():
    ...
    if not pdca_file.exists():
        print("❌ pdca_guardian.pyが見つかりません")
        return
    ...
    # バックアップ作成
    backup_file = Path(f'pdca_guardian.py.backup_{datetime.now().strftime("%Y%m%d_%H%M%S")}')
    with open(backup_file, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"📋 バックアップ作成: {backup_file}")

    # 各種追加を実行
    content = add_violation_types(content)
    content = add_check_methods(content)
    content = add_integration_calls(content)
    ...
    print("\n✅ PDCAガーディアンの更新完了")
    print("\n次のステップ:")
    print("1. python3 verify_missing_rules_implementation.py で実装を検証")
    print("2. python3 rule_sync_system.py で同期状態を再確認")
