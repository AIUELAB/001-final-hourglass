...
class NotificationIntegrator:
    ...
    def has_notification_imports(self, content: str) -> bool:
        ...
        return ("notification_integration" in content or 
                "notification_system" in content)
    ...
    def find_main_functions(self, content: str) -> List[Tuple[str, int, int]]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                ...
                if (node.name in ['main', 'run', 'execute', 'process'] or
                    node.name.endswith('_main') or
                    'main' in node.name.lower()):
    ...
    def add_function_decorator(self, content: str, func_name: str, decorator: str) -> str:
        ...
        for i, line in enumerate(lines):
            if line.strip().startswith(f'def {func_name}('):
                ...
                decorator_line = i
                while decorator_line > 0 and (lines[decorator_line - 1].strip().startswith('@') or
                                               not lines[decorator_line - 1].strip()):
                    decorator_line -= 1
