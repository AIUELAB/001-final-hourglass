...
def assign_highlights():
    ...
    if missing_importance > 0:
        ...
        df['importance_score_temp'] = df.apply(
            lambda row: row['episode_importance_score'] if not pd.isna(row['episode_importance_score'])
    ...
    print(f"  ハイライト選定数: {validation['highlights_assigned']:,}件 ({validation['highlights_assigned'] / validation['total_episodes'] * 100:.1f}%)")
    print()
...
def show_sample_highlights(n: int = 10):
    ...
    for person_id in sample_persons:
        ...
        for _, episode in person_episodes.iterrows():
            ...
            slot_str = str(slot)
            print(f"  {is_highlight} {rank:7s} | {method:20s} | {age:.0f}歳 (slot {slot_str}) | {episode['episode_text'][:60]}...")
        print()
