...
BACKUP_PATH = Path(__file__).parent.parent / f"MASTER_EPISODES_CURRENT_backup_before_episode_fame_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def main():
    ...
    print("算出されたスコア:")
    for r in results:
        print(f"  {r['episode_id']}: fame={r['episode_fame_score']:.1f}, importance={r['episode_importance_score']:.2f}")
    print()
