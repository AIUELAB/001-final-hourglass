...
# カテゴリ別キーワード辞書をインポート
from app.utils.keyword_dictionaries import (
...
BACKUP_PATH = PROJECT_ROOT / f"MASTER_EPISODES_CURRENT_backup_before_fame_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def calculate_episode_fame_score_v2(row: pd.Series) -> Tuple[float, Dict]:
    ...
    details['matched_keywords'] = (
        keyword_result['matched_ultra_famous'] +
        keyword_result['matched_famous']
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='エピソード有名度スコア算出（v2 - カテゴリ別キーワード対応）'
    )
    parser.add_argument(
        '--save',
        action='store_true',
        help='実際にCSVに保存する（デフォルトはdry run）'
    )
    parser.add_argument(
        '--sample',
        type=str,
        help='特定のperson_idのみ処理'
    )
    ...
    if args.sample:
        ...
        for idx, row in target_df.iterrows():
            ...
            print(f"     内訳: base={details['base_score']}, type={details['type_bonus']}, "
