...
def extract_todos(file_path: Path) -> list[tuple[int, str, str, str]]:
    ...
    for tag in TODO_TAGS:
        ...
        patterns.append((tag, re.compile(rf"\b{tag}\b[:\s]*(.*)$", re.IGNORECASE)))
    ...
    for line_no, line in enumerate(lines, 1):
        for tag, pattern in patterns:
            ...
            if match:
                ...
                if (
                    "!" in content[:5]
                    or "urgent" in content.lower()
                    or "critical" in content.lower()
                ):
                    priority = "high"
                elif (
                    "?" in content[:5]
                    or "maybe" in content.lower()
                    or "consider" in content.lower()
                ):
                    priority = "low"
...
def format_todo_report(todos_by_file: dict) -> list[str]:
    ...
    if todos_by_file:
        ...
        sorted_files = sorted(
            todos_by_file.items(),
            key=lambda x: max(
                (1 if t[2] == "high" else 2 if t[2] == "normal" else 3 for t in x[1]), default=3
            ),
        )
        ...
        for file_path, todos in sorted_files:
            ...
            for line_no, tag, priority, content in todos:
                priority_emoji = (
                    "🔴" if priority == "high" else "🟡" if priority == "normal" else "🟢"
                )
                report.append(
...
def main():
    ...
    has_high_priority = any(
        any(todo[2] == "high" for todo in todos) for todos in todos_by_file.values()
    )
