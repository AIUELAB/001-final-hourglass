...
class RuleMerger:
    ...
    def load_rules_registry(self, path: Path) -> None:
        ...
        for rule_id, rule_data in data.items():
            if not rule_id.startswith('RULE_') and not rule_id.startswith('ENTITY_TYPE_') and not rule_id.startswith('FORMAT_'):
                continue
    ...
    def _merge_rules(self, existing: Dict, new: Dict, rule_id: str) -> Dict:
        ...
        # source_filesをマージ
        merged['source_files'] = list(set(
            existing.get('source_files', []) + new.get('source_files', [])
        ))
    ...
    def save_unified_rules(self, output_path: Path) -> None:
        ...
        print(f"  Sources merged: 3 files")
