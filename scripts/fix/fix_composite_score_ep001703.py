...
BACKUP_PATH = (
    ...
    / f"MASTER_EPISODES_CURRENT_backup_before_ep001703_fix_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
)
...
def main():
    ...
    if correct_composite_score is None:
        print("  ❌ エラー: 7軸スコアが不完全のため計算できません")
        return

    print(f"  計算結果: {correct_composite_score:.6f}")
    print(f"  現在の値: {target_row['composite_score']}")
    print(
        f"  差分: {target_row['composite_score'] - correct_composite_score:.6f} (約{target_row['composite_score'] / correct_composite_score:.1f}倍)"
    )
    print()
    ...
    ishinomori_all = df_verify[df_verify["person_name"] == "石ノ森章太郎"][
        ["episode_id", "age", "composite_score"]
    ].sort_values("age")
