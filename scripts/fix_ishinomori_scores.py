...
BACKUP_PATH = (
    ...
    / f"MASTER_EPISODES_CURRENT_backup_before_ishinomori_fix_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
)
...
def main():
    ...
    for _, row in verify_df.iterrows():
        ...
        print(
            f"  {status} {episode_id}: fame={fame_score:.1f if has_fame else 'N/A'}, composite={composite_score:.2f if has_composite else 'N/A'}"
        )
