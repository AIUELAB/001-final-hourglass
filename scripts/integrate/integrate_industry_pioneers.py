#!/usr/bin/env python3
"""
産業界功績者エピソードをMASTER_EPISODES_CURRENT.csvに統合するスクリプト

使用方法:
    python3 integrate_industry_pioneers.py
"""
...
# ファイルパス
INDUSTRY_CSV = "generated/industry_pioneers_episodes.csv"
...
def main():
    print("=" * 80)
    print("産業界功績者エピソード統合スクリプト")
    print("=" * 80)

    # 産業界功績者CSV読み込み
    try:
        with open(INDUSTRY_CSV, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            industry_rows_dict = list(reader)
            industry_header = reader.fieldnames
    except FileNotFoundError:
        print(f"❌ 産業界功績者CSVが見つかりません: {INDUSTRY_CSV}")
        sys.exit(1)

    print(f"\n産業界功績者エピソード: {len(industry_rows_dict)}件")
    ...
    for row_dict in industry_rows_dict:
