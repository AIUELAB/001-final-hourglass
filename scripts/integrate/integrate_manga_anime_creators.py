#!/usr/bin/env python3
"""
漫画家・アニメクリエイターエピソードをMASTER_EPISODES_CURRENT.csvに統合するスクリプト

使用方法:
    python3 integrate_manga_anime_creators.py
"""
...
# ファイルパス
MANGA_ANIME_CSV = "generated/manga_anime_creators_episodes.csv"
...
def main():
    print("=" * 80)
    print("漫画家・アニメクリエイターエピソード統合スクリプト")
    print("=" * 80)

    # 漫画家・アニメCSV読み込み
    try:
        with open(MANGA_ANIME_CSV, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            manga_anime_rows_dict = list(reader)
            manga_anime_header = reader.fieldnames
    except FileNotFoundError:
        print(f"❌ 漫画家・アニメCSVが見つかりません: {MANGA_ANIME_CSV}")
        sys.exit(1)

    print(f"\n漫画家・アニメクリエイターエピソード: {len(manga_anime_rows_dict)}件")
    ...
    for row_dict in manga_anime_rows_dict:
    ...
    if added_count > 0:
        ...
        if award_level_idx != -1:
            ...
            historical_count = sum(1 for row in master_rows if row[award_level_idx] == "HISTORICAL")
            contemporary_art_count = sum(1 for row in master_rows if row[award_level_idx] == "CONTEMPORARY_ART")
            edo_scholar_count = sum(1 for row in master_rows if row[award_level_idx] == "EDO_SCHOLAR")
            traditional_arts_count = sum(1 for row in master_rows if row[award_level_idx] == "TRADITIONAL_ARTS")
            domestic_sports_count = sum(1 for row in master_rows if row[award_level_idx] == "DOMESTIC_SPORTS")
            entertainment_count = sum(1 for row in master_rows if row[award_level_idx] == "ENTERTAINMENT")
            manga_anime_count = sum(1 for row in master_rows if row[award_level_idx] == "MANGA_ANIME")
            ...
            print(f"HISTORICAL: {historical_count}件")
            print(f"CONTEMPORARY_ART: {contemporary_art_count}件")
            print(f"EDO_SCHOLAR: {edo_scholar_count}件")
            print(f"TRADITIONAL_ARTS: {traditional_arts_count}件")
            print(f"DOMESTIC_SPORTS: {domestic_sports_count}件")
            print(f"ENTERTAINMENT: {entertainment_count}件")
            print(f"MANGA_ANIME: {manga_anime_count}件")
