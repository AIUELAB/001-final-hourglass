#!/usr/bin/env python3
"""
最高峰国際賞エピソードをMASTER_EPISODES_CURRENT.csvに統合するスクリプト

使用方法:
    python3 integrate_top_awards_episodes.py
"""
...
# ファイルパス
TOP_AWARDS_CSV = "generated/top_international_awards_episodes.csv"
...
def is_valid_episode(row_dict: Dict[str, str]) -> bool:
    ...
    # award_levelがINTERNATIONAL_TOPかチェック
    award_level = row_dict.get("award_level", "")
    if award_level != "INTERNATIONAL_TOP":
        print(f"   ⚠️  警告（award_level={award_level}、期待値=INTERNATIONAL_TOP）")
...
def main():
    print("=" * 80)
    print("最高峰国際賞エピソード統合スクリプト")
    print("=" * 80)

    # 最高峰国際賞CSV読み込み
    try:
        with open(TOP_AWARDS_CSV, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            top_awards_rows_dict = list(reader)
            top_awards_header = reader.fieldnames
    except FileNotFoundError:
        print(f"❌ 最高峰国際賞CSVが見つかりません: {TOP_AWARDS_CSV}")
        sys.exit(1)

    print(f"\n最高峰国際賞エピソード: {len(top_awards_rows_dict)}件")
    ...
    for row_dict in top_awards_rows_dict:
