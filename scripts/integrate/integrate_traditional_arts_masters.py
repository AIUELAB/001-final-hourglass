#!/usr/bin/env python3
"""
伝統芸能（人間国宝）エピソードをMASTER_EPISODES_CURRENT.csvに統合するスクリプト

使用方法:
    python3 integrate_traditional_arts_masters.py
"""
...
# ファイルパス
TRADITIONAL_ARTS_CSV = "generated/traditional_arts_masters_episodes.csv"
...
def main():
    print("=" * 80)
    print("伝統芸能（人間国宝）エピソード統合スクリプト")
    print("=" * 80)

    # 伝統芸能CSV読み込み
    try:
        with open(TRADITIONAL_ARTS_CSV, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            traditional_arts_rows_dict = list(reader)
            traditional_arts_header = reader.fieldnames
    except FileNotFoundError:
        print(f"❌ 伝統芸能CSVが見つかりません: {TRADITIONAL_ARTS_CSV}")
        sys.exit(1)

    print(f"\n伝統芸能（人間国宝）エピソード: {len(traditional_arts_rows_dict)}件")
    ...
    for row_dict in traditional_arts_rows_dict:
