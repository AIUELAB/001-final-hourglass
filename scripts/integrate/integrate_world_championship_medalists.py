#!/usr/bin/env python3
"""
世界選手権金メダリストエピソードをMASTER_EPISODES_CURRENT.csvに統合するスクリプト

使用方法:
    python3 integrate_world_championship_medalists.py
"""
...
# ファイルパス
WORLD_CHAMPIONSHIP_CSV = "generated/world_championship_medalists_episodes.csv"
...
def main():
    print("=" * 80)
    print("世界選手権金メダリストエピソード統合スクリプト")
    print("=" * 80)

    # 世界選手権金メダリストCSV読み込み
    try:
        with open(WORLD_CHAMPIONSHIP_CSV, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            world_championship_rows_dict = list(reader)
            world_championship_header = reader.fieldnames
    except FileNotFoundError:
        print(f"❌ 世界選手権金メダリストCSVが見つかりません: {WORLD_CHAMPIONSHIP_CSV}")
        sys.exit(1)

    print(f"\n世界選手権金メダリストエピソード: {len(world_championship_rows_dict)}件")
    ...
    for row_dict in world_championship_rows_dict:
