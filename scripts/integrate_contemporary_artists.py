#!/usr/bin/env python3
"""
現代芸術家エピソードをMASTER_EPISODES_CURRENT.csvに統合するスクリプト

使用方法:
    python3 integrate_contemporary_artists.py
"""
...
# ファイルパス
CONTEMPORARY_ARTISTS_CSV = "generated/contemporary_artists_episodes.csv"
...
def main():
    print("=" * 80)
    print("現代芸術家エピソード統合スクリプト")
    print("=" * 80)

    # 現代芸術家CSV読み込み
    try:
        with open(CONTEMPORARY_ARTISTS_CSV, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            contemporary_artists_rows_dict = list(reader)
            contemporary_artists_header = reader.fieldnames
    except FileNotFoundError:
        print(f"❌ 現代芸術家CSVが見つかりません: {CONTEMPORARY_ARTISTS_CSV}")
        sys.exit(1)

    print(f"\n現代芸術家エピソード: {len(contemporary_artists_rows_dict)}件")
    ...
    for row_dict in contemporary_artists_rows_dict:
