#!/usr/bin/env python3
"""
文化芸術系国際賞エピソードをMASTER_EPISODES_CURRENT.csvに統合するスクリプト

使用方法:
    python3 integrate_cultural_arts_episodes.py
"""
...
# ファイルパス
CULTURAL_ARTS_CSV = "generated/cultural_arts_international_awards_episodes.csv"
...
def is_valid_episode(row_dict: Dict[str, str]) -> bool:
    ...
    # award_levelがINTERNATIONALかチェック
    award_level = row_dict.get("award_level", "")
    if award_level != "INTERNATIONAL":
        print(f"   ⚠️  警告（award_level={award_level}、期待値=INTERNATIONAL）")

    # 人生の節目タグが「受賞」かチェック
    milestone_tag = row_dict.get("人生の節目タグ", "")
    if milestone_tag != "受賞":
        print(f"   ⚠️  警告（人生の節目タグ={milestone_tag}、期待値=受賞）")
...
def main():
    print("=" * 80)
    print("文化芸術系国際賞エピソード統合スクリプト")
    print("=" * 80)

    # 文化芸術系国際賞CSV読み込み
    try:
        with open(CULTURAL_ARTS_CSV, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            cultural_arts_rows_dict = list(reader)
            cultural_arts_header = reader.fieldnames
    except FileNotFoundError:
        print(f"❌ 文化芸術系国際賞CSVが見つかりません: {CULTURAL_ARTS_CSV}")
        sys.exit(1)

    print(f"\n文化芸術系国際賞エピソード: {len(cultural_arts_rows_dict)}件")
    ...
    for row_dict in cultural_arts_rows_dict:
        person_name = row_dict.get("person_name", "不明")
        age = row_dict.get("age", "?")
        award_name = row_dict.get("award_level", "")

        print(f"\nチェック: {person_name} ({age}歳, {award_name})...", end=" ")
