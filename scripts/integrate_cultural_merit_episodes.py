...
ER_EPISODES_CURRENT.csvに統合するスクリプト

使用方法:
    python3 integrate_cultural_merit_episodes.py
"""
...
# ファイルパス
CULTURAL_MERIT_CSV = "generated/cultural_merit_awards_episodes.csv"
...
def main():
    print("=" * 80)
    print("文化勲章受章者エピソード統合スクリプト")
    print("=" * 80)

    # 文化勲章CSV読み込み
    try:
        with open(CULTURAL_MERIT_CSV, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            cultural_merit_rows_dict = list(reader)
            cultural_merit_header = reader.fieldnames
    except FileNotFoundError:
        print(f"❌ 文化勲章CSVが見つかりません: {CULTURAL_MERIT_CSV}")
        sys.exit(1)

    print(f"\n文化勲章エピソード: {len(cultural_merit_rows_dict)}件")
    ...
    for row_dict in cultural_merit_rows_dict:
        ...
        # award_levelを"NATIONAL"から"CULTURAL_MERIT"に修正
