#!/usr/bin/env python3
"""
近代スポーツ（国内）エピソードをMASTER_EPISODES_CURRENT.csvに統合するスクリプト

使用方法:
    python3 integrate_domestic_sports_legends.py
"""
...
def main():
    print("=" * 80)
    print("近代スポーツ（国内）エピソード統合スクリプト")
    print("=" * 80)

    # 近代スポーツCSV読み込み
    try:
        with open(DOMESTIC_SPORTS_CSV, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            domestic_sports_rows_dict = list(reader)
            domestic_sports_header = reader.fieldnames
    except FileNotFoundError:
        print(f"❌ 近代スポーツCSVが見つかりません: {DOMESTIC_SPORTS_CSV}")
        sys.exit(1)

    print(f"\n近代スポーツ（国内）エピソード: {len(domestic_sports_rows_dict)}件")
    ...
    for row_dict in domestic_sports_rows_dict:
