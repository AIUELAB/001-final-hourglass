#!/usr/bin/env python3
"""
芸能・エンターテインメントエピソードをMASTER_EPISODES_CURRENT.csvに統合するスクリプト

使用方法:
    python3 integrate_entertainment_legends.py
"""
...
# ファイルパス
ENTERTAINMENT_CSV = "generated/entertainment_legends_episodes.csv"
...
def main():
    print("=" * 80)
    print("芸能・エンターテインメントエピソード統合スクリプト")
    print("=" * 80)

    # 芸能CSV読み込み
    try:
        with open(ENTERTAINMENT_CSV, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            entertainment_rows_dict = list(reader)
            entertainment_header = reader.fieldnames
    except FileNotFoundError:
        print(f"❌ 芸能CSVが見つかりません: {ENTERTAINMENT_CSV}")
        sys.exit(1)

    print(f"\n芸能・エンターテインメントエピソード: {len(entertainment_rows_dict)}件")
    ...
    for row_dict in entertainment_rows_dict:
