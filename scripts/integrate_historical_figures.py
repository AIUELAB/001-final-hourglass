#!/usr/bin/env python3
"""
歴史的人物エピソードをMASTER_EPISODES_CURRENT.csvに統合するスクリプト

使用方法:
    python3 integrate_historical_figures.py
"""
...
# ファイルパス
HISTORICAL_CSV = "generated/historical_figures_episodes.csv"
...
def main():
    print("=" * 80)
    print("歴史的人物エピソード統合スクリプト")
    print("=" * 80)

    # 歴史的人物CSV読み込み
    try:
        with open(HISTORICAL_CSV, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            historical_rows_dict = list(reader)
            historical_header = reader.fieldnames
    except FileNotFoundError:
        print(f"❌ 歴史的人物CSVが見つかりません: {HISTORICAL_CSV}")
        sys.exit(1)

    print(f"\n歴史的人物エピソード: {len(historical_rows_dict)}件")
    ...
    for row_dict in historical_rows_dict:
