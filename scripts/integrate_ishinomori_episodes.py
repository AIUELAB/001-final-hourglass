#!/usr/bin/env python3
"""
石ノ森章太郎エピソードをMASTER_EPISODES_CURRENT.csvに統合するスクリプト

使用方法:
    python3 integrate_ishinomori_episodes.py
"""
...
# ファイルパス
ISHINOMORI_CSV = "generated/ishinomori_shotaro_episodes.csv"
...
def is_valid_episode(row_dict: Dict[str, str]) -> bool:
    ...
    if not episode_text or len(episode_text.strip()) < 100:
        print(f"   ❌ 無効（episode_text短すぎる: {len(episode_text)}文字）")
        return False
...
def main():
    print("=" * 80)
    print("石ノ森章太郎エピソード統合スクリプト")
    print("=" * 80)

    # 石ノ森章太郎CSV読み込み
    try:
        with open(ISHINOMORI_CSV, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            ishinomori_rows_dict = list(reader)
            ishinomori_header = reader.fieldnames
    except FileNotFoundError:
        print(f"❌ 石ノ森章太郎CSVが見つかりません: {ISHINOMORI_CSV}")
        sys.exit(1)

    print(f"\n石ノ森章太郎エピソード: {len(ishinomori_rows_dict)}件")
    ...
    for row_dict in ishinomori_rows_dict:
        person_name = row_dict.get("person_name", "不明")
        age = row_dict.get("age", "?")
        episode_type = row_dict.get("episode_type", "")

        print(f"\nチェック: {person_name} ({age}歳, {episode_type})...", end=" ")
