#!/usr/bin/env python3
"""
文学賞受賞者エピソードをMASTER_EPISODES_CURRENT.csvに統合するスクリプト

使用方法:
    python3 integrate_literary_awards.py
"""
...
# ファイルパス
LITERARY_AWARDS_CSV = "generated/literary_awards_episodes.csv"
...
def main():
    print("=" * 80)
    print("文学賞受賞者エピソード統合スクリプト")
    print("=" * 80)

    # 文学賞受賞者CSV読み込み
    try:
        with open(LITERARY_AWARDS_CSV, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            literary_rows_dict = list(reader)
            literary_header = reader.fieldnames
    except FileNotFoundError:
        print(f"❌ 文学賞受賞者CSVが見つかりません: {LITERARY_AWARDS_CSV}")
        sys.exit(1)

    print(f"\n文学賞受賞者エピソード: {len(literary_rows_dict)}件")
    ...
    for row_dict in literary_rows_dict:
        ...
        # award_levelを"NATIONAL"から"LITERARY_AWARD"に修正
        if row_dict.get("award_level", "") == "NATIONAL":
            row_dict["award_level"] = "LITERARY_AWARD"
