#!/usr/bin/env python3
"""
国民栄誉賞エピソードをMASTER_EPISODES_CURRENT.csvに統合するスクリプト

使用方法:
    python3 integrate_national_honor_episodes.py
"""
...
# ファイルパス
NATIONAL_HONOR_CSV = "generated/national_honor_awards_episodes.csv"
...
def main():
    print("=" * 80)
    print("国民栄誉賞エピソード統合スクリプト")
    print("=" * 80)

    # 国民栄誉賞CSV読み込み
    try:
        with open(NATIONAL_HONOR_CSV, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            national_honor_rows_dict = list(reader)
            national_honor_header = reader.fieldnames
    except FileNotFoundError:
        print(f"❌ 国民栄誉賞CSVが見つかりません: {NATIONAL_HONOR_CSV}")
        sys.exit(1)

    print(f"\n国民栄誉賞エピソード: {len(national_honor_rows_dict)}件")
    ...
    for row_dict in national_honor_rows_dict:
