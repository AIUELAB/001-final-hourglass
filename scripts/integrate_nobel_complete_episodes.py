#!/usr/bin/env python3
"""
ノーベル賞完全版エピソードをMASTER_EPISODES_CURRENT.csvに統合するスクリプト

使用方法:
    python3 integrate_nobel_complete_episodes.py
"""
...
# ファイルパス
COMPLETE_CSV = "generated/nobel_prize_complete_episodes.csv"
...
def is_valid_episode(row_dict: Dict[str, str]) -> bool:
    ...
    # award_levelがNOBELかチェック
    award_level = row_dict.get("award_level", "")
    if award_level != "NOBEL":
        print(f"   ⚠️  警告（award_level={award_level}、期待値=NOBEL）")
...
def main():
    print("=" * 80)
    print("ノーベル賞完全版エピソード統合スクリプト")
    print("=" * 80)

    # 完全版CSV読み込み
    try:
        with open(COMPLETE_CSV, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            complete_rows_dict = list(reader)
            complete_header = reader.fieldnames
    except FileNotFoundError:
        print(f"❌ 完全版CSVが見つかりません: {COMPLETE_CSV}")
        sys.exit(1)

    print(f"\n完全版エピソード: {len(complete_rows_dict)}件")
    ...
    for row_dict in complete_rows_dict:
