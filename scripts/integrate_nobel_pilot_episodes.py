#!/usr/bin/env python3
"""
ノーベル賞パイロット版エピソードをMASTER_EPISODES_CURRENT.csvに統合するスクリプト

使用方法:
    python3 integrate_nobel_pilot_episodes.py
"""
...
# ファイルパス
PILOT_CSV = "generated/nobel_prize_pilot_episodes.csv"
...
def main():
    ...
    try:
        with open(PILOT_CSV, "r", encoding="utf-8-sig") as f:
            ...
            pilot_rows_dict = list(reader)
            pilot_header = reader.fieldnames
    except FileNotFoundError:
        print(f"❌ パイロット版CSVが見つかりません: {PILOT_CSV}")
        sys.exit(1)

    print(f"\nパイロット版エピソード: {len(pilot_rows_dict)}件")
    ...
    for row_dict in pilot_rows_dict:
