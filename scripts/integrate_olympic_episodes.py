#!/usr/bin/env python3
"""
オリンピック金メダリストエピソードをMAST ER_EPISODES_CURRENT.csvに統合するスクリプト

使用方法:
    python3 integrate_olympic_episodes.py
"""
...
# ファイルパス
OLYMPIC_CSV = "generated/olympic_gold_medalists_episodes.csv"
...
def main():
    print("=" * 80)
    print("オリンピック金メダリストエピソード統合スクリプト")
    print("=" * 80)

    # オリンピック金メダリストCSV読み込み
    try:
        with open(OLYMPIC_CSV, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            olympic_rows_dict = list(reader)
            olympic_header = reader.fieldnames
    except FileNotFoundError:
        print(f"❌ オリンピック金メダリストCSVが見つかりません: {OLYMPIC_CSV}")
        sys.exit(1)

    print(f"\nオリンピック金メダリストエピソード: {len(olympic_rows_dict)}件")
    ...
    for row_dict in olympic_rows_dict:
        ...
        # award_levelは既に"OLYMPIC"のためそのまま使用
