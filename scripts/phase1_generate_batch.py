    ...
    def generate_batch(self, max_retries: int = 2, skip_existing: bool = True):
        ...
        for idx, person in enumerate(persons, 1):
            ...
            for attempt in range(max_retries + 1):
                ...
                try:
                    success = self.generator.generate_single_episode(
                        person_id,
                        auto_approve=True
                    )
    ...
    def save_results(self):
        ...
        output = {
            'metadata': {
                ...
                'total_persons': len(self.results['success']) + len(self.results['failed']) + len(self.results['skipped']),
                'success_count': len(self.results['success']),
                'failed_count': len(self.results['failed']),
                'skipped_count': len(self.results['skipped']),
            },
...
def main():
    ...
    try:
        ...
        success_rate = len(generator.results['success']) / (
            len(generator.results['success']) + len(generator.results['failed'])
        ) * 100 if (len(generator.results['success']) + len(generator.results['failed'])) > 0 else 0
