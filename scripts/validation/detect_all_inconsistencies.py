...
def check_name_text_consistency(row) -> list[dict[str, Any]]:
    """B. 本文とフィールドの不一致チェック"""
...
def check_fictional_canon(row) -> list[dict[str, Any]]:
    """D. 架空キャラのカノン整合チェック"""
    issues: list[dict[str, Any]] = []
...
def main():
    ...
    for row in episodes:
        ...
        for issue in a_issues:
            categories["A_age_year"].append({"episode_id": ep_id, "person_name": person_name, **issue})

        # B. 名前・本文整合
        b_issues = check_name_text_consistency(row)
        for issue in b_issues:
            categories["B_name_text"].append({"episode_id": ep_id, "person_name": person_name, **issue})

        # C. 事実関係
        c_issues = check_factual_issues(row)
        for issue in c_issues:
            categories["C_factual"].append({"episode_id": ep_id, "person_name": person_name, **issue})

        # D. 架空キャラ
        d_issues = check_fictional_canon(row)
        for issue in d_issues:
            categories["D_fictional"].append({"episode_id": ep_id, "person_name": person_name, **issue})
