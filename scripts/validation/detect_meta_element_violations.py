...
META_PATTERNS: dict[str, list[str]] = {
    ...
    # 受賞・評価
    # RCA-20260201 Phase14: 「評価を受け」パターンを削除（誤検出多数）
    # - 「上司からの評価」「組織内評価」など物語内表現と区別困難
    # - ベストセラー・受賞・ランキングパターンのみ維持
    "受賞・評価": [
    ...
    # RCA-20260201 Phase14: 「シリーズの」を除外（物語内用語として使用される）
    # - 「EVAシリーズの同調実験」など作品内機体名・用語として自然
    # - メタ的言及（「シリーズで」「シリーズにおいて」等）のみ検出
    "シリーズ・続編言及": [
        r"[『「].+?[』」]シリーズ",
        r"(?:シリーズ|続編|前作|次回作|劇場版|OVA|OAD)(?:で|において|が|は|を通じて)",
    ],
    "制作年言及": [
