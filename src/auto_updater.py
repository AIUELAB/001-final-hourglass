...
class AutoUpdater:
    ...
    def atomic_sheet_update(
        ...
        try:
            ...
            # 3. フォーマット設定
            requests.extend(self._create_format_requests(
                self._get_sheet_id(spreadsheet_id, sheet_name),
                len(all_values),
                len(header)
            ))
    ...
    def force_full_replacement(
        ...
        try:
            ...
            # 2. 一時シートにデータ書き込み
            success, result = self.atomic_sheet_update(
                spreadsheet_id, temp_sheet_name, df, clear_cache=True
            )
