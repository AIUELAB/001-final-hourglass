...
class HeadlessExecutor:
    ...
    async def execute_task(self, 
                          ...
                          files: Optional[List[str]] = None) -> TaskResult:
    ...
    async def _run_optimize(self, params: Dict, files: List[str]) -> Dict:
        ...
        if params.get("profile"):
            cmd = ["py-spy", "record", "-o", "profile.svg", "--", "python", "-m", "pytest", "tests/"]
    ...
    async def execute_workflow(self, 
                              tasks: List[TaskType],
                              params: Optional[Dict] = None,
                              files: Optional[List[str]] = None) -> List[TaskResult]:
        ...
        if self.parallel and self.executor:
            ...
            for task in tasks:
                future = asyncio.create_task(
                    self.execute_task(task, params, files)
                )
...
@click.command()
@click.option('-t', '--task', multiple=True, 
