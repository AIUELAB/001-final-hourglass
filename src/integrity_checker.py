...
class IntegrityChecker:
    ...
    def _load_rules(self) -> Dict[str, Any]:
        ...
        default_rules = {
            "required_columns": [
                "person_id", "person_name", "person_name_display", 
                "person_name_ja", "birth_year"
            ],
        ...
        for col in numeric_columns:
            if col in df.columns:
                non_numeric = df[col].apply(
                    lambda x: not pd.isna(x) and not str(x).replace('.', '').isdigit()
                )
                if non_numeric.any():
        ...
        for col in self.rules["unique_columns"]:
            if col in df.columns:
                ...
                if duplicates.any():
                    ...
                    self._add_error(
                        f"カラム '{col}' に重複が{dup_count}件あります: {list(dup_values)}"
                    )
                    is_valid = False
        ...
        for col, pattern in self.rules["patterns"].items():
            if col in df.columns:
                ...
                invalid = df[col].apply(
                    lambda x: not pd.isna(x) and x != "" and not pattern_re.match(str(x))
                )
                if invalid.any():
                    count = invalid.sum()
                    examples = df[invalid][col].head(5).tolist()
                    self._add_warning(
                        f"カラム '{col}' のパターン違反が{count}件: {examples}"
                    )
            ...
            if col in df.columns:
                ...
                if "min" in constraints:
                    ...
                    if below_min.any():
                        count = below_min.sum()
                        self._add_warning(
                            f"カラム '{col}' に最小値({constraints['min']})未満が{count}件"
                        )
                ...
                if "max" in constraints:
                    ...
                    if above_max.any():
                        count = above_max.sum()
                        self._add_warning(
                            f"カラム '{col}' に最大値({constraints['max']})超過が{count}件"
                        )
    ...
    def _check_duplicate_patterns(self, df: pd.DataFrame) -> bool:
        ...
        for col in ["person_name", "person_name_display", "person_name_ja"]:
            if col in df.columns:
                has_spaces = df[col].apply(
                    lambda x: bool(space_pattern.search(str(x))) if pd.notna(x) else False
                )
                if has_spaces.any():
    ...
    def _check_consistency(self, df: pd.DataFrame) -> bool:
        ...
        if "person_name" in df.columns and "person_name_display" in df.columns:
            ...
            completely_different = df.apply(
                lambda row: (
                    pd.notna(row["person_name"]) and 
                    pd.notna(row["person_name_display"]) and
                    len(str(row["person_name"])) > 0 and
                    len(str(row["person_name_display"])) > 0 and
                    not any(part in str(row["person_name_display"]) 
                           for part in str(row["person_name"]).split())
                ),
                axis=1
            )
    ...
    def _auto_repair(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "person_name_display" in df_repaired.columns:
            ...
            if mask.any():
                df_repaired.loc[mask, "person_name_display"] = \
                    df_repaired.loc[mask, "person_name_display"].str.replace(
