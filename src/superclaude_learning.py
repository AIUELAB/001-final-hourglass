...
class LearningSystem:
    ...
    def learn_frequent_operation(self, project_path: str, operation: Dict[str, Any]) -> None:
        ...
        # 既存の操作を更新または新規追加
        existing = next((op for op in knowledge.frequent_operations 
                        if op.get("command") == operation.get("command")), None)
    ...
    def _calculate_similarity(self, context: Dict[str, Any], pattern: Pattern) -> float:
        ...
        if pattern.tags:
            ...
            tag_overlap = len(set(context_tags) & set(pattern.tags)) / max(len(set(context_tags) | set(pattern.tags)), 1)
    ...
    def get_learning_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "frequently_used_patterns": sorted(
                [(p.pattern_id, p.usage_count) for p in self.patterns_cache.values()],
                key=lambda x: x[1],
                reverse=True
            )[:5],
...
if __name__ == "__main__":
    ...
    entry_id = learner.learn_from_success(
        context=context,
        action="pytest tests/",
    ...
    # プロジェクト規約を学習
    learner.learn_project_convention(
        "/test/project",
        "test_command",
        "pytest tests/ -v"
    )
