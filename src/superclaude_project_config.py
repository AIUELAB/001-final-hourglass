...
class ProjectConfigManager:
    ...
    def get_active_config(self, project_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        if "disabled_flags" in project_dict:
            merged["default_flags"] = [
                f for f in merged.get("default_flags", []) if f not in project_dict["disabled_flags"]
            ]

        # 無効化されたMCPサーバーを除外
        if "disabled_mcp_servers" in project_dict:
            merged["enabled_mcp_servers"] = [
                s for s in merged.get("enabled_mcp_servers", []) if s not in project_dict["disabled_mcp_servers"]
            ]
    ...
    def _create_project_config(self, project_path: str, config_data: Dict[str, Any]) -> ProjectConfig:
        ...
        config = ProjectConfig(project_name=project_name, project_path=project_path)
...
if __name__ == "__main__":
    ...
    test_config = manager.create_project_config(
        "/test/ultra-think",
        {
            ...
            "auto_sync": True,
            "auto_test": True,
            "code_style": {"indent": 2, "quotes": "single", "semicolons": False},
            "tool_shortcuts": {
