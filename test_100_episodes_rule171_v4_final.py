#!/usr/bin/env python3
"""
Phase 3: 全100エピソード最終評価 v4（基準スコア微調整 + 乗数拡充）
...
def estimate_category_v4(episode_text: str) -> str:
    """エピソードテキストからカテゴリを推定（v4 = v3と同じ）"""
    text = episode_text
...
print("=" * 80)
print("Phase 3: 全100エピソード最終評価 v4（基準スコア微調整 + 乗数拡充）")
print("=" * 80)
print(f"評価日時: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
print(f"評価基準: 象徴性スコア100点以上")
print(f"改善施策:")
print(f"  1. カテゴリ推定優先順位の見直し（v3継続）")
...
for i, episode in enumerate(episodes, 1):
    ...
    # v4カテゴリ推定
    category = estimate_category_v4(episode_text)

    # RULE_171で評価（基準スコア微調整済み）
    try:
...
print()
print("=" * 80)
print("📊 Phase 3 全100エピソード最終評価結果 v4")
print("=" * 80)
...
print(f"v4（基準調整+拡充）: {pass_count}% ({pass_count}件) {pass_count-69:+d}%")
print()
...
if failed_episodes:
    ...
    print(f"{'='*80}")
    for ep in failed_episodes[:15]:  # 最初の15件表示
        print(f"{ep['id']} - {ep['person']}")
        print(f"  スコア: {ep['score']:.1f}点 (不足: {100-ep['score']:.1f}点)")
        print(f"  カテゴリ: {ep['category']}")
        print()

    if len(failed_episodes) > 15:
        print(f"... 他 {len(failed_episodes) - 15}件")
...
# 結果をCSV保存
output_path = Path('evaluation_results_100_rule171_v4_final_20251002.csv')
...
if pass_count >= 98:
    ...
    print("3. さらなる乗数キーワード拡充")
else:
    print("1. カテゴリ推定のさらなる改善")
    print("2. 基準スコアの追加調整検討")
print("=" * 80)
