...
class TestEntityTypeValidator(unittest.TestCase):
    ...
    def setUp(self):
        """各テストの前処理"""
        self.known_groups = {"サカナクション", "X JAPAN", "SEKAI NO OWARI", "BTS", "GLAY", "B'z", "Mr.Children", "嵐"}
    ...
    def test_all_known_groups_fail(self):
        ...
        test_groups = ["サカナクション", "X JAPAN", "SEKAI NO OWARI", "BTS", "GLAY", "B'z", "Mr.Children", "嵐"]
...
class TestIntegration(unittest.TestCase):
    ...
    def test_ep028_regression_future_achievement_contamination(self):
        ...
        old_ep028 = {
            ...
            "person_name": "又吉直樹",
            "episode_age": 23,
            "episode_text": "あなたと同じ23歳のとき、又吉直樹（23歳）はお笑いコンビ「ピース」を綾部祐二と結成し、M-1グランプリで準優勝を果たし賞金500万円を獲得。当時から文学への傾倒が深く、小説の習作を重ねながら芸人活動を続けた。その後、お笑い芸人の世界を描いた『火花』で第153回芥川龍之介賞を受賞し累計300万部を突破する大ヒット。続く『劇場』も50万部のベストセラーとなり、芸人と作家の二足のわらじを履く稀有な存在として文芸界に新風を吹き込んだ。",
            "category": "エンターテインメント",
        }
        ...
        # 未来の成果混入は失格
        # 注: CONTENT_005の実装が不完全なため、現在はFORMAT_006のみ検出
        # TODO: CONTENT_005（未来実績汚染）チェックを実装
        self.assertFalse(result.is_valid)
        self.assertEqual(result.severity, Severity.CRITICAL)
        # 現状はFORMAT_006が検出される（冒頭フォーマット違反は修正済み）
        # 将来的にはCONTENT_005も検出されるべき
...
if __name__ == "__main__":
    ...
    parser.add_argument("--regression-only", action="store_true", help="EP010リグレッションテストのみを実行")
